...
JSONNode ReadJson(const char* file_contents)
{
    ...
    const char* ch = file_contents;
    bool child_is_first = false;
    size_t current_line = 1;

    while(*ch)
    {
        while(isspace(*ch)) { if(*ch == '\n'){ current_line++; } ch++; } // Maybe implement a custom is space function to comply with JSON standard
        ...
        if(*ch == '"') // Encountered string
        {
            ...
            switch(state)
            {
                case LEXSTATE::SEARCH_VALUE:
                    // puts("Setting node type to string");
        ...
        if(isNumber(ch, &number_buffer)) // Encountered number
        {
            ...
            // puts("Setting node type to number");
        ...
        {
            // puts("Setting node type to object");
        ...
        {
            // printf("Setting node type to array");
            ...
            // If the next character is a closing brace dont allocate memory just continue
            if(*JumpSpace(ch) == ']')
            {
            ...
            // printf(" and generating child node\n");
        ...
        if(*ch == ']')
        {
            ...
            // End of array what next?
            state = LEXSTATE::AWAIT_NEXT;
        }

        if(*ch == 't') // Check if the word is true
        {
            // puts("Setting node type to true");
        ...
        {
            // puts("Setting node type to false");
        ...
        {
            // puts("Setting node type to null");
        ...
        {
            // puts("Closing node of type object");
            if(state == LEXSTATE::AWAIT_NEXT)
            {
                // We are at the end of the current object and should return to parent
                current_node = current_node->parent; // Always do one iteration
            }
        ...
        if(*ch == ',')
        {
            if(state != LEXSTATE::AWAIT_NEXT)
            {
                printf("Invalid state @ comma [Line: %lu, State: %u]\n", current_line, static_cast<unsigned int>(state));
                exit(1);
            }
        ...
        if(*ch == ':')
        {
            ...
            {
                printf("Invalid state @ colon [Line: %lu, State: %u]\n", current_line, static_cast<unsigned int>(state));
                printf("Next character: %.5s\n", ch + 1);
                exit(1);
            }
        ...
        // Error here, unexpected token / char / etc.
    }

    if(current_node != &root) // If we are not back at root parsing was unsuccessful
    {
